...
struct Runtime {
    ...
    bool eal_init(const std::vector<std::string>& args) {
        ...
        argv.push_back(const_cast<char*>("djinn-dpdk"));
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("dpdk_available", []() { return djinn::runtime().dpdk_available(); });
    m.def("gpudev_available", []() { return djinn::runtime().gpudev_available(); });
    m.def("eal_init", [](const std::vector<std::string>& args) { return djinn::runtime().eal_init(args); });
    m.def("create_default_pool", [](const std::string& name, unsigned n, unsigned data_room) {
        return djinn::runtime().create_default_pool(name, n, data_room);
    });
    m.def("alloc_mbuf", [](std::uintptr_t pool_ptr) { return djinn::runtime().alloc_mbuf(pool_ptr); });
}

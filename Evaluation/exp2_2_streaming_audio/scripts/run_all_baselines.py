#!/usr/bin/env python3
"""
Run Experiment 2.2 (Streaming Audio) with all 4 baselines:
1. native_pytorch - Local execution (upper bound)
2. semantic_blind - Djinn remote with all semantic features disabled
3. partially_aware - Djinn remote with model caching but no state persistence
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-id", default="openai/whisper-large-v3", help="HF model identifier")
    parser.add_argument("--chunk-seconds", type=float, default=5.0, help="Audio chunk duration in seconds")
    parser.add_argument("--stride-seconds", type=float, default=1.0, help="Stride between chunks")
    parser.add_argument("--dummy-audio", type=float, default=30.0, help="Generate synthetic audio of N seconds")
    parser.add_argument("--max-new-tokens", type=int, default=64, help="Max tokens per chunk")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("Evaluation/exp2_2_streaming_audio/results"))
...
def run_native_baseline(args: argparse.Namespace) -> Path:
    ...
    script_path = REPO_ROOT / "Evaluation/exp2_2_streaming_audio/scripts/run_local_streaming_baseline.py"
    ...
    cmd = [
        ...
        "--stride-seconds", str(args.stride_seconds),
        "--dummy-audio", str(args.dummy_audio),
    ...
    output_files = sorted(
        output_dir.glob("streaming_audio_native_pytorch_*.json"),
...
def run_remote_baseline(args: argparse.Namespace, baseline_name: str, semantic_aware: bool, use_session: bool = True) -> Path:
    ...
    # Find the output file
    output_files = sorted(
        output_dir.glob(f"streaming_audio_{baseline_name}_*.json"),
        key=lambda p: p.stat().st_mtime,
    )
    if output_files:
        return output_files[-1]
...
def generate_comparison_report(results: Dict[str, Path], args: argparse.Namespace) -> Dict[str, Any]:
    ...
    report = {
        "experiment": "exp2_2_streaming_audio",
            ...
            for run in data["runs"]:
                if "chunks" in run:
                    for chunk in run["chunks"]:
                        all_chunk_latencies.append(chunk.get("latency_ms", 0))
                        total_data_transfer += chunk.get("host_to_device_mb", 0) + chunk.get("device_to_host_mb", 0)
                all_total_times.append(run.get("total_time_ms", 0))
            
            import statistics
            report["baselines"][baseline_name] = {
                "mean_chunk_latency_ms": statistics.mean(all_chunk_latencies) if all_chunk_latencies else None,
...
def main():
    ...
    if len(results) >= 2:
        ...
        for baseline_name, baseline_data in report["baselines"].items():
            print(f"\n{baseline_name}:")
            print(f"  Mean chunk latency: {baseline_data['mean_chunk_latency_ms']:.2f} ms")
